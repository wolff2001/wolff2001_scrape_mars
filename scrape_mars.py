...
def scrape():
    ...
    WILL_VARIABLE=""
    found=False
    for main_section in mars_weather:
        weather_tweet=main_section.find_all('span',class_="css-901oao css-16my406 r-1qd0xha r-ad9z0x r-bcqeeo r-qvutc0")
        for tweet in weather_tweet:
            if found!=True:
                if tweet.text.startswith("InSight"):

                    WILL_VARIABLE=tweet.text
                    print("-"*500)
                    found=True
                else:
                    continue
